...
pub async fn run(
    ...
    loop {
        ...
        let params = PARAMS.parse().map_err(|e| {
            log::error!("Failed to parse noise params: {e}");
            e
        })?;
        ...
        let noise = builder
            ...
            .map_err(|e| {
                log::error!("Failed to add private key: {e}");
                e
            })?
            .psk(3, &gateway_config.auth)
            .map_err(|e| {
                log::error!("Failed to specify PSK: {e}");
                e
            })?
            .build_initiator()
            .map_err(|e| {
                log::error!("Failed to build initiator: {e}");
                e
            })?;
        ...
        .await
        {
            ...
            Err(e) => {
                log::info!("Noise handshake error: {e}");
                Timer::after(Duration::from_millis(backoff_ms)).await;
                backoff_ms = (backoff_ms * 2).min(MAX_BACKOFF_SECS * 1000);
                continue;
            }
        ...
        'sending: loop {
            ...
            times += 1;
            log::info!("Send succesfully {times} times!");
